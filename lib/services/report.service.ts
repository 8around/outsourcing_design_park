...
export class ReportService {
   ...
   */
  async upsertReportConfig(config: Partial<WeeklyReportConfig>): Promise<WeeklyReportConfig> {
    try {
  ...
  async getReportStatistics(): Promise<{
    ...
    try {
      ...
      const stats = {
        ...
        successfulSent: data?.filter(r => r.send_status === 'sent').length || 0,
        failedSent: data?.filter(r => r.send_status === 'failed').length || 0,
        pendingSent: data?.filter(r => r.send_status === 'pending').length || 0,
  ...
  async testEmailSending(testEmail: string): Promise<{ success: boolean; message: string }> {
    try {
      // Here you would call an Edge Function to send a test email
