...
class LogService {
  ...
  async createManualLog(
    data: Omit<CreateLogRequest, "log_type">
   ...
   */
  async createApprovalRequestLog(
    data: CreateApprovalRequestLog & { attachments?: AttachmentFile[] }
  ) {
    const supabase = createClient();
    ...
    if (approvalError) {
      ...
      // 실패 시 생성된 로그도 삭제
      await supabase.from("history_logs").delete().eq("id", historyLog.id);
    ...
    try {
      await supabase.from("notifications").insert({
        ...
        kakao_sent_at: kakaoSendResult?.success
  ...
  async getFilteredLogs(
    ...
    if (filter.target_user_id)
      query = query.eq("target_user_id", filter.target_user_id);
