...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (!error) {
      ...
      redirectTo.searchParams.set("error_message", error.message || "인증에 실패했습니다");
      redirectTo.searchParams.delete("code");
      return NextResponse.redirect(redirectTo);
    }
  }
  ...
  if (error) {
    ...
    if (error === "access_denied" && errorDescription?.includes("expired")) {
      ...
      redirectTo.searchParams.set("error_message", "인증 링크가 만료되었습니다. 다시 시도해주세요.");
    } else {
      redirectTo.pathname = "/login";
      redirectTo.searchParams.set("error", "verification_failed");
      redirectTo.searchParams.set("error_message", errorDescription || "인증에 실패했습니다");
    }
